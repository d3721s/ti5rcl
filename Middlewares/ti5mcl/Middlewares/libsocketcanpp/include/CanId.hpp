...
public: // +++ Operators +++
    ...
    {
        return m_identifier;    //!< Returns the raw CAN ID value.
    }
    ...
    {
        return m_identifier & CAN_ERR_MASK;
    }
    ...
    {
        return m_identifier == static_cast<canid_t>
    ...
    {
        return static_cast<canid_t>(x) >
               m_identifier;    //!< Compares this ID to a 32-bit integer.
    }
...
public: // +++ Getters +++
    ...
    {
        return isErrorFrame(
                   m_identifier);    //!< Indicates whether or not this ID is an error frame.
    }
    constexpr bool hasRtrFrameFlag() const
    {
        return isRemoteTransmissionRequest(
                   m_identifier);    //!< Indicates whether or not this ID is a remote transmission request.
    }
    constexpr bool isStandardFrameId() const
    {
        return !isExtendedFrame(
                   m_identifier);    //!< Indicates whether or not this ID is a standard frame ID.
    }
    constexpr bool isExtendedFrameId() const
    {
        return isExtendedFrame(
                   m_identifier);    //!< Indicates whether or not this ID is an extended frame ID.
    }
