...
typedef int (*list_callback)(const char* name,
                             struct dirent* entry,
                             void* user);
typedef int (*vprint_callback)(char* buff,
                               int maxlen, void* userptr,
...
static inline void _tlog_spin_lock(
    unsigned int* lock)
{
    while (1)
    {
...
static inline void _tlog_spin_unlock(
    unsigned int* lock)
{
...
static struct tm* _tlog_localtime(time_t* timep,
                                  struct tm* tm)
{
...
static int _tlog_getmtime(struct tlog_time*
                          log_mtime, const char* file)
{
    struct tm tm;
    struct stat sb;
    if (stat(file, &sb) != 0)
...
static int _tlog_gettime(struct tlog_time*
                         cur_time)
{
    struct tm tm;
...
void tlog_set_maxline_size(struct tlog_log* log,
                           int size)
{
    if (log == NULL)
    {
...
void tlog_logcount(struct tlog_log* log,
                   int count)
{
    if (log == NULL)
    {
...
void tlog_set_permission(struct tlog_log* log,
                         mode_t file, mode_t archive)
{
    log->file_perm = file;
...
void tlog_set_private(tlog_log* log,
                      void* private_data)
{
    if (log == NULL)
    {
...
static int _tlog_root_default_format(char* buff,
                                     int maxlen,
...
static int _tlog_root_log_buffer(char* buff,
                                 int maxlen, void* userptr,
                                 const char* format, va_list ap)
{
    int len = 0;
    ...
    struct tlog_info_inter* info_inter =
        (struct tlog_info_inter*)userptr;
    ...
    if (tlog.root->segment_log)
    {
        ...
        len += sizeof(*log_head);
        memcpy(&log_head->info, &info_inter->info,
               sizeof(log_head->info));
    }
    max_format_len = maxlen - len - 2;
    buff[maxlen - 1] = 0;
    log_len = tlog.root_format(buff + len,
                               max_format_len, &info_inter->info,
                               info_inter->userptr, format, ap);
    if (log_len < 0)
    {
        return -1;
    }
...
static int _tlog_print_buffer(char* buff,
                              int maxlen, void* userptr,
                              const char* format, va_list ap)
{
    int len;
...
static int _tlog_vprintf(struct tlog_log* log,
                         vprint_callback print_callback,
                         void* userptr, const char* format, va_list ap)
{
    int len;
    int maxlen = 0;
    ...
    len = print_callback(buff, sizeof(buff), userptr,
                         format, ap);
    if (len <= 0)
    {
        return -1;
    ...
    do
    {
        ...
        if (maxlen < log->max_line_size)
        {
            ...
            int ret = pthread_cond_wait(&log->client_cond,
                                        &log->lock);
            log->waiters--;
    ...
    if (log->segment_log)
    {
        segment_head = (struct tlog_segment_head*)(
                           log->buff + log->end);
...
int tlog_vprintf(struct tlog_log* log,
                 const char* format, va_list ap)
{
    return _tlog_vprintf(log, _tlog_print_buffer,
                         NULL, format, ap);
}

int tlog_printf(struct tlog_log* log,
                const char* format, ...)
{
    int len;
    va_list ap;
...
int tlog_stdout_with_color(tlog_level level,
...
static int _tlog_early_print(struct
                             tlog_info_inter* info_inter,
                             const char* format, va_list ap)
{
    ...
    {
        tlog.tlog_early_print(&info_inter->info, format,
                              ap);
        return out_len;
    }
    if (tlog.early_print_no_prefix == 0)
    {
        len = snprintf(log_buf, sizeof(log_buf),
                       "[%.4d-%.2d-%.2d %.2d:%.2d:%.2d,%.3d][%5s]",
                       cur_time.year, cur_time.mon, cur_time.mday,
                       cur_time.hour, cur_time.min,
                       cur_time.sec, cur_time.usec / 1000,
                       tlog_get_level_string(info_inter->info.level));
    }
    out_len = len;
    len = vsnprintf(log_buf + out_len,
                    sizeof(log_buf) - out_len - 1, format, ap);
    out_len += len;
    if (len <= 0)
    {
        return -1;
    ...
    {
        len = tlog.early_print_output(&info_inter->info,
                                      log_buf, out_len,
                                      tlog.early_print_userptr);
    ...
    {
        unused = tlog_stdout_with_color(
                     info_inter->info.level, log_buf, out_len);
    }
    else
    {
...
int tlog_vext(tlog_level level, const char* file,
              ...
              void* userptr, const char* format, va_list ap)
{
    ...
    return _tlog_vprintf(tlog.root,
                         _tlog_root_log_buffer, &info_inter, format, ap);
}

int tlog_ext(tlog_level level, const char* file,
             int line, const char* func,
             void* userptr, const char* format, ...)
{
    ...
    va_list ap;
    va_start(ap, format);
    len = tlog_vext(level, file, line, func, userptr,
                    format, ap);
    va_end(ap);
    return len;
}

static int _tlog_rename_logfile(struct tlog_log*
                                log, const char* log_file)
{
    ...
    snprintf(archive_file, sizeof(archive_file),
             "%s/%s-%.4d%.2d%.2d-%.2d%.2d%.2d%s",
             log->logdir, log->logname, logtime.year,
             logtime.mon, logtime.mday,
    ...
    while (access(archive_file, F_OK) == 0)
    {
        i++;
        snprintf(archive_file, sizeof(archive_file),
                 "%s/%s-%.4d%.2d%.2d-%.2d%.2d%.2d-%d%s",
                 log->logdir, log->logname, logtime.year,
                 logtime.mon,
                 logtime.mday, logtime.hour, logtime.min,
                 logtime.sec, i, log->suffix);
    }
...
static int _tlog_list_dir(const char* path,
...
static int _tlog_count_log_callback(
    const char* path, struct dirent* entry,
    void* userptr)
{
    struct count_log* count_log = (struct count_log*)
                                  userptr;
    ...
    snprintf(logname, sizeof(logname), "%s-",
             log->logname);
...
static int _tlog_get_oldest_callback(
    const char* path, struct dirent* entry,
    void* userptr)
{
    struct stat sb;
    char filename[TLOG_BUFF_LEN];
    struct oldest_log* oldestlog = (struct
                                    oldest_log*)userptr;
    ...
    if (oldestlog->mtime == 0
            || oldestlog->mtime > sb.st_mtime)
    {
        oldestlog->mtime = sb.st_mtime;
        strncpy(oldestlog->name, entry->d_name,
                sizeof(oldestlog->name));
        oldestlog->name[sizeof(oldestlog->name) - 1] =
            '\0';
        return 0;
    }
    return 0;
...
static int _tlog_remove_oldestlog(struct tlog_log*
                                  log)
{
    ...
    if (_tlog_list_dir(log->logdir,
                       _tlog_get_oldest_callback, &oldestlog) != 0)
    {
        return -1;
    }
    char filename[PATH_MAX * 2];
    snprintf(filename, sizeof(filename), "%s/%s",
             log->logdir, oldestlog.name);
    /* delete */
    unlink(filename);
    return 0;
}

static int _tlog_remove_oldlog(struct tlog_log*
                               log)
{
    ...
    if (_tlog_list_dir(log->logdir,
                       _tlog_count_log_callback, &count_log) != 0)
    {
        fprintf(stderr,
                "tlog: get log file count failed.\n");
        return -1;
    }
    /* remove last N log files */
    for (i = 0; i < count_log.lognum - log->logcount;
            i++)
    {
...
static void _tlog_log_unlock(struct tlog_log* log)
{
    ...
    snprintf(lock_file, sizeof(lock_file),
             "%s/%s.lock", log->logdir, log->logname);
    unlink(lock_file);
    close(log->fd_lock);
    log->fd_lock = -1;
}
...
static int _tlog_log_lock(struct tlog_log* log)
{
    ...
    {
        fprintf(stderr,
                "tlog: create lock file failed, %s",
                strerror(errno));
        return -1;
    }
...
static void _tlog_wait_pid(struct tlog_log* log,
                           int wait_hang)
{
    int status;
    if (log->zip_pid <= 0)
    ...
    /* rename zipped file */
    snprintf(gzip_file, sizeof(gzip_file),
             "%s/%s.pending.gz", log->logdir,
             log->logname);
...
static int _tlog_archive_log_compressed(
    struct tlog_log* log)
{
    ...
    snprintf(gzip_file, sizeof(gzip_file),
             "%s/%s.pending.gz", log->logdir,
             log->logname);
    snprintf(pending_file, sizeof(pending_file),
             "%s/%s.pending", log->logdir,
             log->logname);
    ...
    if (log->zip_pid <= 0)
    {
        ...
        {
            _tlog_close_all_fd();
            execl(tlog.gzip_cmd, tlog.gzip_cmd, "-1",
                  pending_file, NULL);
            fprintf(stderr,
                    "tlog: execl gzip failed, no compress\n");
            log->nocompress = 1;
            _exit(1);
        }
        else if (pid < 0)
        {
            ...
            {
                fprintf(stderr,
                        "tlog: vfork failed, errno: %d, no compress\n",
                        errno);
                log->nocompress = 1;
            }
            goto errout;
...
static int _tlog_archive_log_nocompress(
    struct tlog_log* log)
{
    char log_file[TLOG_BUFF_LEN];
    char pending_file[TLOG_BUFF_LEN];
...
static void _tlog_get_log_name_dir(
    struct tlog_log* log)
{
    ...
    pthread_mutex_lock(&tlog.lock);
    strncpy(log_file, log->pending_logfile,
            sizeof(log_file) - 1);
    log_file[sizeof(log_file) - 1] = '\0';
    strncpy(log->logdir, dirname(log_file),
            sizeof(log->logdir) - 1);
    log->logdir[sizeof(log->logdir) - 1] = '\0';
    strncpy(log_file, log->pending_logfile, PATH_MAX);
    log_file[sizeof(log_file) - 1] = '\0';
    strncpy(log->logname, basename(log_file),
            sizeof(log->logname) - 1);
    log->logname[sizeof(log->logname) - 1] = '\0';
    pthread_mutex_unlock(&tlog.lock);
}

static int _tlog_write_screen(struct tlog_log*
                              log, struct tlog_loginfo* info,
                              const char* buff, int bufflen)
{
    if (bufflen <= 0)
    {
    ...
    return tlog_stdout_with_color(info->level, buff,
                                  bufflen);
}

static int _tlog_write_ext(struct tlog_log* log,
                           struct tlog_loginfo* info,
                           const char* buff, int bufflen)
{
    int len;
    ...
    if (log->filesize > log->logsize
            && log->zip_pid <= 0)
    {
        if (log->filesize < lseek(log->fd, 0, SEEK_END)
                && log->multi_log == 0)
        {
            const char* msg =
                "[Auto enable multi-process write mode, log may be lost, please enable multi-process write mode manually]\n";
            log->multi_log = 1;
    ...
    if (log->fd <= 0)
    {
        ...
        if (_tlog_mkdir(log->logdir) != 0)
        {
            ...
            log->print_errmsg = 0;
            fprintf(stderr,
                    "tlog: create log dir %s failed, %s\n",
                    log->logdir,
                    strerror(errno));
            if (errno == EACCES && log->logscreen == 0)
            {
                fprintf(stderr,
                        "tlog: no permission to write log file, output log to console\n");
...
static inline int _tlog_write(struct tlog_log*
                              log, const char* buff,
                              int bufflen)
{
...
int tlog_write(struct tlog_log* log,
               const char* buff, int bufflen)
{
...
static int _tlog_close(struct tlog_log* log,
                       int wait_hang)
{
...
static struct tlog_log* _tlog_next_log(
    struct tlog_log* last_log)
{
    if (last_log == NULL)
    {
        return tlog.log;
...
static struct tlog_log* _tlog_wait_log_locked(
    struct tlog_log* last_log)
{
    int ret = 0;
    struct timespec tm;
    ...
    if (need_wait_pid != 0)
    {
        ret = pthread_cond_timedwait(&tlog.cond,
                                     &tlog.lock, &tm);
    }
    else
    {
...
static void _tlog_wakeup_waiters(struct tlog_log*
                                 log)
{
    pthread_mutex_lock(&log->lock);
    if (log->waiters > 0)
...
static int _tlog_write_output_func(
    struct tlog_log* log, char* buff,
    int bufflen)
{
...
static void _tlog_write_one_segment_log(
    struct tlog_log* log, char* buff,
    int bufflen)
{
    struct tlog_segment_head* segment_head = NULL;
    ...
    for (write_len = 0; write_len < bufflen;)
    {
        ...
        _tlog_write_output_func(log, segment_head->data,
                                segment_head->len - 1);
        write_len += segment_head->len + sizeof(
                         *segment_head);
        segment_head = (struct tlog_segment_head*)(
                           buff + write_len);
    }
...
static void _tlog_write_segments_log(
    struct tlog_log* log, int log_len,
    int log_extlen)
{
    _tlog_write_one_segment_log(log,
                                log->buff + log->start, log_len);
    if (log_extlen > 0)
    {
        /* write extend buffer log */
        _tlog_write_one_segment_log(log, log->buff,
                                    log_extlen);
    }
}

static void _tlog_write_buff_log(struct tlog_log*
                                 log, int log_len,
                                 int log_extlen)
{
    _tlog_write_output_func(log,
                            log->buff + log->start, log_len);
    if (log_extlen > 0)
    {
        /* write extend buffer log */
        _tlog_write_output_func(log, log->buff,
                                log_extlen);
    }
}

static void _tlog_work_write(struct tlog_log* log,
                             int log_len, int log_extlen,
                             int log_dropped)
{
    /* write log */
    if (log->segment_log)
    {
        _tlog_write_segments_log(log, log_len,
                                 log_extlen);
    }
    else
    {
    ...
    if (log_dropped > 0)
    {
        ...
        {
            memset(dropmsg, 0,
                   sizeof(struct tlog_segment_log_head));
            log_head = (struct tlog_segment_log_head*)dropmsg;
            msg += sizeof(struct tlog_segment_log_head);
            log_head->info.level = TLOG_WARN;
        }
        int len = snprintf(msg, msg - dropmsg,
                           "[Total Dropped %d Messages]\n",
                           log_dropped);
        if (log_head)
        {
            log_head->len = len;
        }
        _tlog_write_output_func(log, dropmsg,
                                strnlen(dropmsg, sizeof(dropmsg)));
    }
}

static int _tlog_root_write_screen_log(
    struct tlog_log* log,
    ...
    int bufflen)
{
    ...
    return _tlog_write_screen(log, info, buff,
                              bufflen);
}

static int _tlog_root_write_log(struct tlog_log*
                                log, const char* buff,
                                int bufflen)
{
    ...
    if (tlog.output_func == NULL)
    {
        if (log->segment_log)
        {
            ...
            _tlog_root_write_screen_log(log, &head->info,
                                        head->data, head->len);
            return _tlog_write_ext(log, &head->info,
                                   head->data, head->len);
        }
        _tlog_root_write_screen_log(log, NULL, buff,
                                    bufflen);
        return _tlog_write(log, buff, bufflen);
    }
    if (log->segment_log && tlog.root == log)
    {
        head = (struct tlog_segment_log_head*)buff;
        _tlog_root_write_screen_log(log, &head->info,
                                    head->data, head->len - 1);
        return tlog.output_func(&head->info, head->data,
                                head->len - 1,
                                tlog_get_private(log));
    }
    _tlog_root_write_screen_log(log, NULL, buff,
                                bufflen);
    memset(&empty_info, 0, sizeof(empty_info));
    return tlog.output_func(&empty_info.info, buff,
                            bufflen, tlog_get_private(log));
}
...
static void* _tlog_work(void* arg)
{
    ...
    while (1)
    {
        ...
        _tlog_work_write(log, log_len, log_extlen,
                         log_dropped);
        pthread_mutex_lock(&tlog.lock);
                           ...
                           int no_prefix, int color)
{
    ...
    tlog.early_print_no_prefix = (no_prefix == 0) ?
                                 0 : 1;
    tlog.early_print_color = (color == 0
                              || isatty(STDOUT_FILENO) ==  0) ? 0 : 1;
}

void tlog_reg_early_printf_callback(
    tlog_early_print_func callback)
{
    tlog.tlog_early_print = callback;
}

void tlog_reg_early_printf_output_callback(
    ...
    int log_screen, void* private_data)
{
    ...
    tlog.early_print_with_screen = (log_screen == 0) ?
                                   0 : 1;
}

const char* tlog_get_level_string(tlog_level
                                  level)
{
    if (level >= TLOG_END)
    {
...
static void _tlog_log_setlogscreen(
    struct tlog_log* log, int enable)
{
    if (log == NULL)
    {
        return;
...
static int _tlog_reg_output_func(tlog_log* log,
...
int tlog_reg_output_func(tlog_log* log,
                         tlog_output_func output)
{
    if (log == tlog.root)
    {
...
int tlog_reg_format_func(tlog_format_func
                         callback)
{
...
static void _tlog_get_gzip_cmd_path(void)
{
    ...
    for (char* tok = strtok_r(copy_path, ":",
                              &save_ptr); tok;
            tok = strtok_r(NULL, ":", &save_ptr))
    {
        snprintf(gzip_cmd_path, sizeof(gzip_cmd_path),
                 "%s/gzip", tok);
        if (access(gzip_cmd_path, X_OK) != 0)
        {
            continue;
        }
        snprintf(tlog.gzip_cmd, sizeof(tlog.gzip_cmd),
                 "%s", gzip_cmd_path);
        break;
    }
    free(copy_path);
...
tlog_log* tlog_open(const char* logfile,
                    ...
                    int buffsize, unsigned int flag)
{
    ...
    log->ext_end = 0;
    log->dropped = 0;
    log->buffsize = (buffsize > 0) ? buffsize :
                    TLOG_BUFF_SIZE;
    log->logsize = (maxlogsize >= 0) ? maxlogsize :
                   TLOG_LOG_SIZE;
    log->logcount = (maxlogcount <= 0) ? 0 :
                    maxlogcount;
    log->fd = -1;
    log->filesize = 0;
    log->zip_pid = -1;
    ...
    log->print_errmsg = 1;
    log->waiters = 0;
    log->block = ((flag & TLOG_NONBLOCK) == 0) ? 1 :
                 0;
    log->nocompress = ((flag & TLOG_NOCOMPRESS) == 0)
                      ? 0 : 1;
    log->logscreen = ((flag & TLOG_SCREEN) == 0) ? 0 :
                     1;
    log->logscreen_color = ((flag & TLOG_SCREEN_COLOR)
                            == 0 ||
                            isatty(STDOUT_FILENO) == 0) ? 0 : 1;
    log->multi_log = ((flag & TLOG_MULTI_WRITE) == 0)
                     ? 0 : 1;
    log->segment_log = ((flag & TLOG_SEGMENT) == 0) ?
                       0 : 1;
    ...
    if (log->nocompress)
    {
        strncpy(log->suffix, TLOG_SUFFIX_LOG,
                sizeof(log->suffix));
    }
    else
    {
        strncpy(log->suffix, TLOG_SUFFIX_GZ,
                sizeof(log->suffix));
    }
    ...
    {
        fprintf(stderr,
                "tlog: malloc log buffer failed, %s\n",
                strerror(errno));
        goto errout;
    }
    pthread_mutex_lock(&tlog.lock);
...
void tlog_rename_logfile(struct tlog_log* log,
                         const char* logfile)
{
    pthread_mutex_lock(&tlog.lock);
    strncpy(log->pending_logfile, logfile,
            sizeof(log->pending_logfile) - 1);
    pthread_mutex_unlock(&tlog.lock);
...
static void tlog_fork_child(void)
{
    ...
    int ret = pthread_create(&tlog.tid, &attr,
                             _tlog_work, NULL);
    if (ret != 0)
    {
        fprintf(stderr,
                "tlog: create tlog work thread failed, %s\n",
                strerror(errno));
        goto errout;
    }
    goto out;
...
int tlog_init(const char* logfile, int maxlogsize,
              int maxlogcount,
              int buffsize, unsigned int flag)
{
    pthread_attr_t attr;
    ...
    if (buffsize > 0
            && buffsize < TLOG_MAX_LINE_SIZE_SET * 2)
    {
        fprintf(stderr,
                "tlog: buffer size is invalid.\n");
        return -1;
    }
    ...
    tlog.run = 1;
    log = tlog_open(logfile, maxlogsize, maxlogcount,
                    buffsize, flag);
    if (log == NULL)
    {
    ...
    if ((flag & TLOG_NOCOMPRESS) == 0
            && tlog.gzip_cmd[0] == '\0')
    {
        fprintf(stderr,
                "tlog: can not find gzip command, disable compress.\n");
    }
    tlog.output_no_prefix = ((flag &
                              TLOG_FORMAT_NO_PREFIX) == 0) ? 0 : 1;
    tlog.root = log;
    tlog.root_format = _tlog_root_default_format;
    ret = pthread_create(&tlog.tid, &attr, _tlog_work,
                         NULL);
    if (ret != 0)
    {
        fprintf(stderr,
    ...
    {
        pthread_atfork(&tlog_fork_prepare,
                       &tlog_fork_parent, &tlog_fork_child);
    }
    return 0;
errout:
    if (tlog.tid)
